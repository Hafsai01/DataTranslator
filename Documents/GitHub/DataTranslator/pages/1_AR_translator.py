...
REQUIRED_COLUMNS = ['code', 'invoice date', 'invoice#', 'due date','retainage','sub-total','grand total']
...
def merge_customer_ids(df_maestro, df_mapping):
    """
    Merge Maestro DataFrame with mapping file to get CUSTOMER_ID without dropping other data.
    """
...
# ----------------- Streamlit UI -----------------
st.title("AR Data Translator")

maestro_file = st.file_uploader("Upload Maestro Excel file", type=["xlsx", "xls"])
mapping_file = st.file_uploader("Upload Customer ID Mapping Excel file", type=["xlsx", "xls"])
...
if maestro_file and mapping_file:
    try:
        ...
        #final_cols = ['Intact_Customer_Id'] + [col.title() for col in REQUIRED_COLUMNS]
